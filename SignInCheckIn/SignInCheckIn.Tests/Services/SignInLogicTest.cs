...
namespace SignInCheckIn.Tests.Services
{
    public class SignInLogicTest
    {
        ...
        private const int MiddleSchoolEventTypeId = 402;
        private const int HighSchoolEventTypeId = 403;
        ...
        public void ShouldGetSingleEventForSignIn()
        {
            ...
            _eventRepository.Setup(r => r.GetEvents(signInTime, signInTime, 1, true, It.IsAny<List<int>>(), true)).Returns(GetTestEventSet());
        ...
        [Test]
        public void ShouldNotSignIntoAcWithCapacityAcRooms()
        {
            // Arrange
            var fourYearOldBirthdate = System.DateTime.Now.AddYears(-5);
        ...
        [Test]
        public void TestStudentMinistrySigninLogic()
        {
            // JPC - this test is basically to make sure we bypass the whole signin logic stuff
            // for MSM/HSM participants. The reason we do not test this path/function for the KC/Childcare
            // participants is explained below
            ...
            // Act
            var result = _fixture.SignInParticipants(participantEventMapDto, GetStudentMinistryTestEventSet());

            // Assert
            Assert.AreEqual(true, result[0].NonRoomSignIn);
        }

        private List<MpEventDto> GetStudentMinistryTestEventSet()
        {
            var currentStartTime = System.DateTime.Now;
        ...
        // This test was created for DE3499
        [Test]
        public void ShouldNotSignIntoAcWithSecondEventEligibleOnly()
        {
            // Arrange
            var fourYearOldBirthdate = System.DateTime.Now.AddYears(-5);
            ...
            // service event, then service event and ac event
            var eventList = GetDE3499TestEventSet();
            ...
            // getting bumping rooms for the eligible rooms
            //_roomRepository.Setup(m => m.GetBumpingRoomsForEventRoom(eventDtos[0].EventId, eventRoomDtos[0].EventRoomId.GetValueOrDefault())).Returns(new List<MpBumpingRoomsDto>());
            //_roomRepository.Setup(m => m.GetBumpingRoomsForEventRoom(eventDtos[1].EventId, eventRoomDtos[1].EventRoomId.GetValueOrDefault())).Returns(new List<MpBumpingRoomsDto>());
        ...
        private List<MpEventDto> GetDE3499TestEventSet()
        {
            // we need to set times dynamically on the test data set, to avoid having unit tests
            // break when run at later or earlier hours - this may still be a trouble spot
            var currentStartTime = System.DateTime.Now;
